...
namespace VisualMountParking.Markers
{
    public class ArucoDetector
    {
        ...
        public Bitmap DrawMarkers(Bitmap image, bool showRejected = false)
        {
        ...
        private void AnalyzeFrame(Mat frame, bool showRejected = false)
        {
            if (!frame.IsEmpty)
            {
                ...
                using (VectorOfVectorOfPointF rejected = new VectorOfVectorOfPointF()) // rejected contours
                {
                    ...
                    if (ids.Size > 0)
                    {
                        ArucoInvoke.DrawDetectedMarkers(frame, corners, ids, new MCvScalar(0, 0, 255));
                        // Detect3DPositions(frame, ids, corners);
                    }
                }
            }
        }

        private static void Detect3DPositions(Mat frame, VectorOfInt ids, VectorOfVectorOfPointF corners)
        {
